...
const createJsonResponse = (
  body: ErrorResponse | Awaited<ReturnType<typeof createRoom>>,
  status: number,
...
const createErrorResponse = (
  ...
  return createJsonResponse(
    { ok: false, error: 'Room creation failed.' },
    500,
    corsHeaders,
  );
};
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const parsedRequest = parseCreateRoomRequest(body);
  if (!parsedRequest.ok) {
    return createJsonResponse(
  ...
  try {
    const response = await createRoom(parsedRequest.data);
    return createJsonResponse(response, 201, corsHeaders);
  } catch (error: unknown) {
    return createErrorResponse(
      error instanceof Error ? error : new Error('Unknown room create failure'),
      corsHeaders,
    );
  }
}
