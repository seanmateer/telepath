...
const createJsonResponse = (
  body: ErrorResponse | Awaited<ReturnType<typeof joinRoom>>,
  status: number,
...
const createErrorResponse = (
  ...
  return createJsonResponse(
    { ok: false, error: 'Room join failed.' },
    500,
    corsHeaders,
  );
};
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const parsedRequest = parseJoinRoomRequest(body);
  if (!parsedRequest.ok) {
    return createJsonResponse(
  ...
  try {
    const response = await joinRoom(parsedRequest.data);
    return createJsonResponse(response, 200, corsHeaders);
  } catch (error: unknown) {
    return createErrorResponse(
      error instanceof Error ? error : new Error('Unknown room join failure'),
      corsHeaders,
    );
  }
}
